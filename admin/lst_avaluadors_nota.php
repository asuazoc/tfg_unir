...
<h2>Notas Comunicación/Evaluadores</h2>
			<p id="page-intro">Notas agrupadas por comunicación</p>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-content">
					...
					<div style="display: block;" class="tab-content default-tab" id="tab1"> <!-- This is the target div. id must match the href of this div's tab -->
							...
							<!-- Page Head -->
<?$sql="SELECT usuari, id FROM users where rol='evaluador' ORDER BY usuari";
...
<table id="pagerTable" cellspacing="1" class="tablesorter">
   <thead>
   	<tr>
   		...
   		<th></th>
		<th></th>
   		<th colspan="<?=$rows?>">Evaluadores</th>
   		<th></th>
   	</tr>
...
$c++;?>
                <tr>
                  ...
                  <td><?=$row['titol']?></td>
							<?
							$sql_usuaris="SELECT usuari,id FROM users where rol='evaluador' ORDER BY usuari";
							$notes=0;
							$sum_notes=0;
							foreach ($con->query($sql_usuaris) as $row_usuaris2){
							   $tComunicacionV = explode(',',$row_usuaris2['comunicacions_visibles']);
							   $aSql = "SELECT notatotal FROM comunicacions_nota WHERE id_comunicacion =".$row['id']." AND user='".$row_usuaris2['usuari']."' ";
