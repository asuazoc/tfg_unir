...
if ($_SERVER['REQUEST_METHOD']=='GET'){
	...
	$result = mysqli_query($con,"select * from `comunicacions_nota` where id_comunicacion = '$id' and user='$user'");
	if (mysqli_num_rows($result) > 0) {
	   mysqli_query($con,"UPDATE `comunicacions_nota` SET nota1='$nota1',nota2='$nota2',nota3='$nota3',nota4='$nota4',nota5='$nota5',nota6='$nota6',notatotal='$notatotal',recomanacio='$recomanacio'  WHERE id_comunicacion = '$id' and user='$user'");
	} else {
		 $sql = "INSERT INTO  `comunicacions_nota` ( `id_comunicacion` ,`user` ,`userid` ,`nota1`,`nota2`,`nota3`,`nota4`,`nota5`,`nota6`,`notatotal`,`recomanacio`) VALUES ('$id','$user','$userid','$nota1','$nota2','$nota3','$nota4','$nota5','$nota6','$notatotal','$recomanacio')";
...
function validateForm()
{
	...
	if(document.getElementById("notatotal").value==''){
		alert('ERROR: Falten camps per poder desar la nota');
		return false;
	}else{
		return true;
	}
...
if(($data>$data_final_avaluacio)&&($_SESSION['revisio']=='N')){
	echo ' style="display:none" ';
}else if(($data>$data_final_revisio)&&($_SESSION['revisio']=='S')){
	echo ' style="display:none" ';
}else if(($data>$data_final_revisio2)&&($_SESSION['revisio']=='S2')){
	echo ' style="display:none" ';
}?>
>
	  ...
	  <tr>
		...
		<input type="text" name="notatotal" <?php echo $nota_total;?> id="notatotal" maxlength="4" size="4"  onfocus="if(this.value == '0.00') { this.value = ''; }" value="<?=$row_nota['notatotal']?>"  onkeypress='validate(event)'/>
	   </td>
	  </tr>
	<tr>
	    	<td>
			...
			<br />
		    	<p>
    				<?=$comunicacio[$row['comunicacio']]['es']?>
		    	</p>
		</td>
	</tr>
	<tr>
			...
			<br />
				...
				<select name="recomanacio">
					<?foreach ($comunicacio_eval as $key => $comunicacion) {?>
					  <option value="<?=$key?>" <?=($row_nota['recomanacio']==$key) ? 'selected':''  ?> ><?=$comunicacion?></option>
					<?}?>
				</select>
		    	</p>
		</td>
